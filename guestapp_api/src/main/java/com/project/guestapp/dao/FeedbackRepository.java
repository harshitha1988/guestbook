...
public class FeedbackRepository {
	...
	public List<Feedback> findAllFeedback() {
		return jdbcTemplate.query("select "
				+ "fb.feedback_id, fb.guest_id , fb.comments, fb.likes, fb.unlikes, fb.created_on, gu.name as guest_name from "
				+ "feedback fb, guestuser gu where gu.guest_id = fb.guest_id ", new FeedbackRowMapper());
	}

	public Feedback findFeedback(int id) {
		return jdbcTemplate.queryForObject(
				"select " + "fb.feedback_id, fb.comments, fb.likes, fb.unlikes, fb.created_on, gu.name as guest_name from "
	...
	public void saveFeedback(Feedback feedback) {
		jdbcTemplate.update(
				...
				feedback.getUser().getUserId(), feedback.getComment(), feedback.getLikes(), feedback.getUnLikes());

	}

	public void updateFeedback(Feedback feedback) {
		jdbcTemplate.update("update feedback set likes = ?, unlikes = ?, updated_on = CURRENT_TIMESTAMP  where feedback_id= ?",
				feedback.getLikes(), feedback.getUnLikes(), feedback.getFeedbackId());
	}
